...
scroll_code = "\n".join(line.split(";", 1)[0] for line in scroll.splitlines())
if "death_timer" in scroll_code:
    errors.append(
        "src/scroll.s: stale death_timer reference from a partial merge; "
        "Phase 13 recovery is driven by game_state/respawn_pending"
    )
...
if ".export respawn_pending, respawn_render_row" not in state:
